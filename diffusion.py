...
class VolaDiff():
    ...
    def denoise_x(self,z,t,x_pred):
        if t<0.03:
            #with the provided noise schedule, the last 3% of the time steps are irrelevant
            return x_pred
    ...
    def sample_uncond(self,batch_size,return_history=False,printing=False):
        ...
        for t in tqdm(torch.arange(self.n_steps,1,-1)*1.) if printing else torch.arange(self.n_steps,1,-1)*1.:
            t=torch.tensor(t).to(self.device)/self.n_steps*self.T_max
    ...
    def sample_cond(self,batch_size,target_data_loader,conditioning_time=0.5,return_history=False,printing=False):
        ...
        subset=torch.randint(0,target_data_loader.data.shape[0],(batch_size,))
        pred_con=target_data_loader.data[subset].to(self.device).view(-1,1,16,32)
